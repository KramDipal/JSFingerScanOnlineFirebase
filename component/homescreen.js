...
const Tab = createBottomTabNavigator();
  ...
  export default function HomeScreenComponent({navigation}) {
    ...
    const [triggerPopScan, setTriggerPopScan] = useState(null);
    const maxAttempts = 3;
    ...
    useEffect(() => {
      ...
      (async () => {
        console.log('check biometric support');
        ...
        //prompt for fingerprint validation upon boot up or clearTokenAndSignOut
    ...
    const initialLogin = async () => {
      ...
      try {
        const userCredential = await signInWithEmailAndPassword(auth, 'kiko@gmail.com', 'test123');
        ...
        navigation.navigate('Profile', {lastLogin, userId});
        // setTriggerPopScan(false);
      } catch (error) {
        console.error(error);
        ...
        if (result.success) {
          ...
          if (storedToken && auth.currentUser) {
            ...
            // Attempt to sign in with stored token (Firebase will refresh it if valid)
                await signInWithEmailAndPassword(auth, 'kiko@gmail.com', 'test123'); // Replace with
